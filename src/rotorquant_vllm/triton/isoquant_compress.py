"""
Triton kernel for IsoQuant compress: tiled matmul with pre-split rotation columns.
...
def _iso_compress_kernel(
    ...
    """Fused IsoQuant compress: norm + normalize + tiled rotation + bucketize + pack.
    ...
    # rotated_even[j] = sum_k x_hat[k] * R^T[k, 2j]
    # rotated_odd[j]  = sum_k x_hat[k] * R^T[k, 2j+1]
...
def iso_compress(
    ...
    if not x.is_cuda:
        return _iso_compress_cpu(x, rot_T_even, rot_T_odd, boundaries)
